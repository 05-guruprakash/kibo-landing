...
export default function Counter({
  ...
  const defaultPlaces = [...value.toString()].map((ch, i, a) => {
    if (ch === '.') {
      return '.';
    } else {
      ...
      return (
        10 **
        (decimalIndex === -1 ? a.length - i - 1 : i < decimalIndex ? decimalIndex - i - 1 : -(i - decimalIndex))
      );
    }
  }) as (number | '.')[];

  const actualPlaces = places || defaultPlaces;
